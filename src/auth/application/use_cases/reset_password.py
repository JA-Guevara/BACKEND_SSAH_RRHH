...
class ResetPassword:
    ...
    async def execute(self, token: str, new_password: str) -> None:
        ...
        await self.user_repository.update_password(stored_token.user_id, hashed_password)
        await self.token_repository.consume_password_reset_token(stored_token.id)
        await self.token_repository.revoke_all_refresh_tokens(stored_token.user_id)
